...
def get_indicators(
    ...
    """Get technical indicator values from Tushare daily data.

    Tushare does not provide built-in technical indicator APIs, so indicators
    are computed from daily OHLCV data using pandas.
    """
    best_ind_params = {
    ...
    try:
        ...
        handle_tushare_error(e, f"daily({ts_code}) for indicator")
...
def _calculate_indicator(df: pd.DataFrame, indicator: str) -> pd.DataFrame:
    """Calculate technical indicator from OHLCV data using pandas."""
    if indicator == "close_50_sma":
        ...
        raise ValueError(f"Indicator {indicator} is not supported by Tushare provider")
